...
- 1回の実行で**2件以上**のイベントを供給したURLを「一覧ページ」とみなして学習する。
  1件しか出ないURLは個別イベントのページで、次回以降は陳腐化するため学習しない
- `known_sources.json` に登録済みのホストは学習しない（重複巡回を避ける）
- ヒットしなかった情報源は `misses` を増やし、**3回連続で0件なら削除**する（更新停止・構成変更への追随）
- 供給実績の多い順に**最大8件**まで保持（入力トークンの膨張を防ぐ）
