# 東海3県 犬イベント自動収集・Googleカレンダー反映システム 計画書

作成日: 2026-08-08（更新版: Instagram連携なし・低コスト構成）

## 1. 目的

東海3県（愛知・岐阜・三重）で開催される犬メインのイベント・マルシェ（例: わんにゃんドーム、犬市場 等）の開催情報を毎月自動収集し、専用のGoogleカレンダーに登録する。各イベントの来場者数は、公式発表があればその数値を、なければClaude APIによる予測値を「予測」と明記した上で備考欄に記載する。
...
- 対象地域: 愛知・岐阜・三重の3県で開催（会場所在地基準。主催・拠点が県外でも東海3県内開催なら対象に含める想定）
  - **2026-08-29 変更**: 当初は静岡県を含む4県を対象としていたが、3県へ縮小した。
    変更時点でカレンダーに登録済みだった静岡県のイベント11件は削除済み（`events.json` 32件→21件）
- **東海3県外の例外（2026-08-14 追加）**: 「インターペット」東京・大阪のみ対象に含める。
...
- 同ページには東海4県（当時）のイベントが11件掲載されていたが、収集できたのは4件。**1ページで7件を取りこぼしていた**
...
しかし同じ実行の検索回数は**合計25回**であり、4県（当時）×対象月の網羅検索・インターペット2件・既知シリーズ確認を含めて
