...
const TOOL = {
  name: 'record_events',
  description: '検索結果・公式サイト本文から、東海3県で開催される犬メインのイベント情報を抽出して記録する',
  input_schema: {
    type: 'object',
    properties: {
      events: {
      ...
      candidates: {
        type: 'array',
        description:
          '東海3県（およびインターペット）で見つけたイベントを、除外したものも含めて全件記録する。' +
...
// web_search の実行回数上限。目標件数を満たすには3県ぶんの検索が要るため多めに確保する。
...
function buildSystem({ today, horizon }) {
  ...
  return `あなたは東海3県（愛知・岐阜・三重）の犬関連イベント情報を収集するアシスタントです。

本日は ${today} です。抽出対象は ${today} 〜 ${horizon}（6ヶ月先）に開催されるイベントに限ります。

手順（この順序で行うこと）:
1. web_search ツールで東海3県の犬イベント情報を検索する。与えられた公式サイト本文だけでは新規イベントを発見できないため、検索は必ず行うこと
2. **除外の判断をする前に**、見つかったイベントを candidates として全件洗い出す。
   複数の都道府県のイベントが並ぶ集約ページ・まとめ記事では、**東海3県のものを1件ずつ拾い漏らさないこと**。
   「開催日：YYYY/MM/DD」形式で列挙されたリストは、上から順に最後まで確認する。
   この段階では除外基準を適用せず、東海3県で開催される犬関連イベントは疑わしいものも含めてすべて挙げる
...
- 3県すべて（愛知・岐阜・三重）を個別に検索し、特定の県に偏らないようにする
- 県名や地域名（東海／愛知／岐阜／三重／名古屋／岐阜市／四日市／豊橋 等）と
...
- 開催地が東海3県（愛知・岐阜・三重）以外のものは対象外とする。**静岡県は対象外**。
  **ただし「インターペット」だけは例外**として、
...
export async function extractEvents({ knownSourcePages, today, horizon }) {
  const userContent = [
    `${today} 時点で、${today} 〜 ${horizon} に東海3県（愛知・岐阜・三重）で開催される犬イベントを調べて記録してください。`,
